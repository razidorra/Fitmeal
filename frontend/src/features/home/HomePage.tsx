...
export function HomePage() {
  ...
  const faqs = [
    ['How does FitMeal create my calorie target?', 'FitMeal uses the Mifflin–St Jeor equation with your age, chosen equation constant, height, weight, activity level and goal. The formula defines male (+5) and female (−161) constants; “Other / prefer not to say” uses −161 and the form states this explicitly.'],
    ['Can FitMeal help me lose weight or gain weight?', 'Yes. Choose lose, maintain or gain when you set up your profile. FitMeal adjusts your daily calorie target and protein goal to suit that direction.'],
    ['What happens after I save my profile?', 'You can generate a daily meal plan with calorie and protein budgets, ask FitMeal AI general meal-planning questions, and log weight check-ins on the Progress page.'],
    ['Are calorie targets exact?', 'They are a practical starting estimate, not an exact prescription. Your needs can vary, so use your energy, hunger and progress over several weeks to guide adjustments.'],
    ['How often should I log my weight?', 'Many people find one or two check-ins per week useful. Focus on the longer trend instead of reacting to normal daily changes from hydration, food and routine.'],
    ['Can I update my goal or body details later?', 'Yes. Open the Meal Planner, choose Edit profile, update your details, and save. Refresh the current plan afterwards so its targets use your newest information.'],
    ['Can one meal be called healthy or unhealthy?', 'Usually, no. A balanced eating pattern matters more than one individual meal. FitMeal supports flexible, nourishing habits without labelling foods as good or bad.'],
    ['Is FitMeal AI a doctor or dietitian?', 'No. The assistant provides general planning and nutrition guidance only. It does not diagnose, prescribe or replace advice from a doctor or registered dietitian.'],
    ['Where does the nutrition data come from?', 'Daily targets use an established energy-estimation formula. Meal calories and protein are target budgets for portion planning, not calculated nutrition for the displayed ingredient quantities. Recipe values are per-serving estimates; check product labels when exact values matter.'],
  ];
  return <>
