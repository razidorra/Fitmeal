...
export function RecipeModal({ recipe, onClose }: { recipe: Recipe; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 bg-[rgba(6,6,5,.82)] backdrop-blur-sm flex items-center justify-center p-8 max-[560px]:p-3 z-100 animate-[recipe-modal-fade_180ms_ease]" onClick={onClose}>
    <div ref={dialogRef} tabIndex={-1} className="bg-surface border border-line rounded-3xl max-w-260 w-full max-h-[88vh] grid grid-cols-2 max-[800px]:grid-cols-1 max-[800px]:max-h-[92vh] max-[800px]:overflow-y-auto overflow-hidden shadow-[0_30px_90px_rgba(0,0,0,.5)]" role="dialog" aria-modal="true" aria-labelledby="recipe-dialog-title" onClick={(event) => event.stopPropagation()}>
      ...
      <div className="relative p-10 overflow-y-auto">
        ...
        <h2 id="recipe-dialog-title" className="font-display font-semibold text-[32px] mt-0 mb-3.5">{recipe.title}</h2>
